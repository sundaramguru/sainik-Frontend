...
//Widow
    const getUsers1 = async () => {
      const sn=localStorage.getItem('Service_No');
      const response = await axiosJWT.get('http://localhost:5000/awidowforma');
...
{/*   -----------------------------------------  HEADER  SECTION ------------------------------------------------- */}
    ...
    {COLSW.map(c => <option key={c}>{c}</option>)}
         ...
         {COLSE.map(c => <option key={c}>{c}</option>)}
              ...
              <button className = "btn btn-secondary right-align"  onClick = {viewesm}>ESM Record </button>&nbsp;&nbsp;&nbsp;

    <button className = "right-align btn btn-secondary "  onClick = {viewwidow}>Widow Record </button>&nbsp;&nbsp;&nbsp;
    <button className = "btn btn-secondary right-align"  onClick = {viewemp}>Employed Record </button><br/>&nbsp;&nbsp;&nbsp;
