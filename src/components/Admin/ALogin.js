...
const ALogin = () => {
  ...
  const Auth = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/alogin`, {
  ...
  const Autodelete = async (e) => {
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/Autodelete`, {
        // Name: Name,
        // Service_No:Service_No,
        // Mobile:Mobile,
        // Mail_Id: Mail_Id,
        ...
        <div id="formContent">
          ...
          <div id="formFooter">
            <a class="underlineHover la">
              {" "}
              <Link to="/AForgetPage"> Forgot Password?</Link>
            </a>
            <br />
            <br />
            <a class="underlineHover la">
