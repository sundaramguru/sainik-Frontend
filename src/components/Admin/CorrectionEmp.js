...
const [Superintendent_Q, setSuperintendent_Q] = useState(localStorage.getItem('Superintendent_Query'));
const [Director_Q, setDirector_Q] = useState(localStorage.getItem('Director_Query'));
...
const ClerkQ = async (e) => {
    ...
    try {
//Send Email
          emailjs.sendForm('service_080xmpx', 'template_gv085xg', e.target, 'PaHWpUgrsCFyqSKRl')
            .then((result) => {
...
//
      ...
      await axios.post('http://localhost:5000/EClerkQ', {
          ClerkQ:Clerk_Q,
...
try {
  ...
  emailjs.sendForm('service_080xmpx', 'template_gv085xg', e.target, 'PaHWpUgrsCFyqSKRl')
    ...
    e.target.reset()
    //
  await axios.post('http://localhost:5000/ESuperintendentQ', {
...
try {
  ...
  await axios.post('http://localhost:5000/EDirectorQ', {
               ...
               (() => {
                   if(ad == "Clerk") {
                           ...
                           <form onSubmit={ClerkQ}>
                             <h2><b>Clerk</b></h2><br/>
                           <textarea rows="4" cols="150" name = "post" value={Clerk_Q }onChange={(e) => setClerk_Q(e.target.value)} />
                           <button className="btn my-2 my-sm-0 " type="submit"><strong>Post</strong> </button>
                           </form>
                             </div>
                           )
                           ...
                           <textarea rows="4" cols="150"  name = "post" value={Superintendent_Q}onChange={(e) => setSuperintendent_Q(e.target.value)} />
                           <button className="btn my-2 my-sm-0 " type="submit"><strong>Post</strong> </button>
                           ...
                           <h2><b>Superintendent</b></h2><br/>
                           <textarea rows="4" cols="150" name = "post" value={user.S_Remark}onChange={(e) => setSuperintendent_Q(e.target.value)} />
