...
const ARegister = () => {
  ...
  const getRSB = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/RSB_D`
    );
    setrsb(response.data);
  };

  // const getZSB = async () => {
  //  const response = await axiosJWT.get(`${process.env.REACT_APP_BACKEND_URL}/ZSB_D`);
  ...
  const getZSB = async () => {
    ...
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/ZSB_D`,
      {
        params: { rsb: sn },
      }
    );
  ...
  const onChange = (e) => {
    ...
    if (
      ...
      re.test(e.target.value.toUpperCase())
    ) {
        ...
        <div id="formContent">
          ...
          <form onSubmit={ARegister}>
            ...
            <input
              type="email"
              id="login"
              class="fadeIn second emailInput"
              name="login"
              placeholder="Email"
              value={email}
              onClick={(e) => validateEmail(e.target.value)}
            ...
            <input
              type="text"
              id="login"
              class="fadeIn second numberInput"
              name="login"
              minlength="10"
              maxlength="12"
              placeholder="Mobile"
                ...
                <div className="row remove-pad ">
                  <div className="col-sm-5">
                    <select
                      ...
                      name="RSB"
                      value={RSB}
                      onChange={(e) => setRSB(e.target.value)}
                      required
                    >
                      <option value="" disabled selected>
                        Select RSB
                ...
                <div className="row remove-pad ">
                  <div className="col-sm-5">
                    ...
                    >
                      ...
                      {zsb.map((user, index) => (
                        <option key={user.id} value={user.ZSB_Name}>
                          {user.ZSB_Name}
                        </option>
                      ))}
                    </select>
