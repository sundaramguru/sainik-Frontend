...
const Single = (props) => {
  ...
  const Single = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/Single`, {
        Item: Item,
        Value: Value,
      });
      // setItem(() => "");
  ...
  const getfRsb = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/F_Single`
    );
