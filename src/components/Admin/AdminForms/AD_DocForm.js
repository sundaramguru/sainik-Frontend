...
const AS_DocForm = () => {
  ...
  const getUsers = async () => {
      ...
      const response = await axiosJWT.get('http://localhost:5000/AD_DocForm',{
        params:{
          A_Service_No: sn,
          A_Name:dn,
          A_Relation:r
          ...
          <div className="text-center text-dark p-3" style={{backgroundColor: "#008E89"}}>
            <label className="header"> Dependent Document  </label>
               <div className = "left-align dis">
                ...
                <label className="sn-mar">{"Dep name :"}</label> <p>{users[6]}</p>
...
<div  className = "right-align ">
 <img src={users[5]} className='docPhoto' width='150'onClick={()=> window.open(users[5], "_blank")} />
 </div>
      ...
      <button onClick={()=> window.open(users[5], "_blank")}>Open image</button>

  </div>

        <div className="text-center text-dark p-3 foot" style={{backgroundColor: "#DBE6FD"}}>
        <button className=" btn" ><Link to="/adminform7a">Back</Link> </button>
        <button className="btn my-2 my-sm-0 " ><Link to="/adminform7">Next</Link> </button>
        </div>
</div>
</div>
