...
const District = () => {
  ...
  const getZsb = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/getZsb`
    );
    setgZsb(response.data);
  };
  const getState = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/state_D`
    );
    setgState(response.data);
  };
  const getfdist = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/F_District`
    );
  ...
  const District = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/I_district`, {
        ZSB_Name: ZSB_Name,
        State: State,
        Dist: Dist,
        Dist_Code: Dist_Code,
        ...
        <div id="formContent">
          <form onSubmit={District}>
            ...
            <div>
              ...
              >
                ...
                {gState.map((user, index) => (
                  <option key={user.id} value={user.State}>
                    {user.State}
                  </option>
                ))}
              </select>
            ...
            <input
              ...
              value={Dist_Code}
              onChange={(e) => setDist_Code(e.target.value)}
              required
            />
            <input
              type="submit"
