...
const State = () => {
  ...
  const getfRsb = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/F_State`
    );
  ...
  const StateS = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/I_state`, {
        RSB_Name: RSB_Name,
        ...
        <div id="formContent">
          <form onSubmit={StateS}>
            ...
            <input
              ...
              id="login"
              class="fadeIn second textInput"
              placeholder="State_Surname"
              value={State_Surname}
              onChange={(e) => setState_Surname(e.target.value)}
              required
            />
