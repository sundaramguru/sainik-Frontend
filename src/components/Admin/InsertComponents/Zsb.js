...
const Zsb = () => {
    ...
    const axiosJWT = axios.create();
        ...
        const getfZsbRsb = async () => {
            const response = await axiosJWT.get('http://localhost:5000/F_fZsbRsb');
...
const columns = [
 ...
 {dataField: 'ZSB_RSB_ID', text: 'ZSB-RSB-Id', sort: true , filter: textFilter()},
  ...
  const pagination = paginationFactory({
    ...
    return (
      ...
      <div id="formContent">
        ...
        <form onSubmit={Zsb}><br/><br/>
        <p className="has-text-centered">{msg}</p><br/><br/>
        <h1 className='insert-pad'><strong>ZSB</strong></h1><br/><br/>
        <div >
        <select id="login"className="fadeIn second textInput" value ={RSB_Name} onChange={(e)=> setRSB_Name(e.target.value)} required>
 <option value = "" disabled selected>--Select RSB Name--</option>
             {gRsb.map((user, index) => (
        ...
        <input type="text" id="login" class="fadeIn second textInput"  placeholder="ZSB_Name"value={ZSB_Name} onChange={(e) => setZSB_Name(e.target.value)} required/>
        <input type="text" id="login" class="fadeIn second textInput"  placeholder="ZSB_Surname"value={ZSB_Surname} onChange={(e) => setZSB_Surname(e.target.value)} required/>
