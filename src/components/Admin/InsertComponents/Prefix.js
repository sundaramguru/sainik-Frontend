...
const axiosJWT = axios.create();
    ...
    const getfzsb = async () => {
    const response = await axiosJWT.get('http://localhost:5000/F_Prefix');
 ...
 {dataField: 'Prefix', text: 'Prefix', sort: true , filter: textFilter()},
  ...
  const pagination = paginationFactory({
    ...
    const Prefix_I = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:5000/I_prefix', {
    ...
    return (
      ...
      <div id="formContent">
        ...
        <h1 className='insert-pad'><strong>Prefix</strong></h1>
        ...
        <input type="text" id="login" class="fadeIn second textInput"  placeholder="Prefix"value={Prefix} onChange={(e) => setPrefix(e.target.value)} required/>
