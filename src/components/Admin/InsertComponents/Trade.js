...
const Trade = () => {
  ...
  const getfRsb = async () => {
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/F_Trade`
    );
  ...
  const Trade = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/I_trade`, {
        Service_Id: Service,
        ...
        <div id="formContent">
          <form onSubmit={Trade}>
            ...
            <input
              ...
              value={Group_Name}
              onChange={(e) => setGroup_Name(e.target.value.toUpperCase())}
              required
            />
            <input
              type="text"
