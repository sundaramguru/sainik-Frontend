     ...
     {/* ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ HEADER ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/}
      ...
      {/* ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ HEADER ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/}
        ...
        <FormLabel text={"Adhaar No."} />
          <div className="col-lg-4 space">
          <input type="text"  class="  formInputunderline  formInput" autocomplete= 'off' maxlength = "12" minLength = "12" name="Adhaar"value={Adhaar} onChange={onAdhaarValidateChange} required/>
          </div>
