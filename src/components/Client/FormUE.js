...
const [Trade_Code, setTrade_Code] = useState(localStorage.getItem('Trade_Code'));
const [CESM_No, setCESM_No] = useState('');
      ...
      const getTrades  = async () => {
            ...
            const getCivilQualification  = async () => {
                    ...
                    setUsers(response.data);
                              ...
                              const getESMs = async () => {
                                      const response = await axiosJWT.get('http://localhost:5000/getESMs',{
...
const FormUE = async (e) => {
    e.preventDefault();
    try {
  await axios.post('http://localhost:5000/FormUE', {
...
// const checkesm = (e) => {
// if(cem!==CESM_No){
//   setErrorCodes('Please Enter a correct ESM Number !! ')
//  }
//
// else{
//   setErrorCodes(' !! ')
...
<input type = "hidden"  class="  formInput" name= "cem"  value={em}/><br/><br/><br/>


    <FormLabel text={"ESM No"} />
    <div className="col-md-4 space ">
    <input type="text"  class="  formInput" autocomplete = "off" maxlength = "20" onClick = {Emp} placeholder ="Enter Full ESM Reg No" name="ESM_No"  value={ESM_No} onChange={(e) => setESM_No(e.target.value)}required /><br/>
    <span style=
           {{
...
{users.map((user, index) => (
  ...
  <div className="row"key={user.id}>
          ...
          <div className="col-md-4 space">
          <input type="text"  class="  formInput" autocomplete = "off" disabled maxlength = "20"  name="Civil_Qualification"  value={user.vCivil_Qualification} onChange={(e) => setCivil_Qualification(e.target.value)}required />

           </div>

                          <FormLabel text={"Trade /Branch"} />
                          <div className="col-md-4 space">
                          <input type="text"  class="  formInput"disabled autocomplete = "off" maxlength = "20"  name="Trade_Name" value={user.vTrade} onChange={(e) => setTrade_Name(e.target.value)} required/>
                          </div>
         ...
         { visiblebutton3 &&
               ...
               <div className="col-md-4 space">
               <input type="text"  class="  formInput" disabled autocomplete = "off" maxlength = "20"  name="Equi_Test" value={Equi_Test} onChange={(e) => setEqui_Test(e.target.value)}required />
               </div>
               ...
               <div className="col-md-4 space">
               <input type="text"  class="  formInput" autocomplete = "off" maxlength = "20"  name="Trade_Code"  onClick = {calAge} value={Trade_Code} onChange={(e) => setTrade_Code(e.target.value.toUpperCase())} />
               </div>
                   </div>
      }
