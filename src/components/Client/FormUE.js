...
const getEDOB  = async () => {
  ...
  const getES  = async () => {
      const response = await axiosJWT.get('http://localhost:5000/getES',{
...
<input type = "hidden"  class="  formInput" name= "cem"  value={em}/><br/><br/><br/>
 ...
 <input type="hidden"  class="  formInput" autocomplete = "off" maxlength = "20"  name="ESM_No" value={ESM_No} onChange={(e) => setESM_No(e.target.value)} />
    <FormLabel text={"ESM No"} />
    <div className="col-md-4 space ">
    <input type="text"  class="  formInput" autocomplete = "off" maxlength = "20" onClick = {Emp}  name="ESM_No"  value={ESM_No} required /><br/>
    <span style=
           {{
