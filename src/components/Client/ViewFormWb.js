...
const ViewFormWEdit = () => {
  ...
  const [Family_Pension, setFamily_Pension] = useState(
    localStorage.getItem("V_Family_Pension")
  );
  const [W_Nxt_Kin, setW_Nxt_Kin] = useState(
    localStorage.getItem("A_W_Nxt_Kin")
  );
  const [Death_Date, setDeath_Date] = useState(
    localStorage.getItem("V_Death_Date")
  );
  const [Death_Nature, setDeath_Nature] = useState(
    localStorage.getItem("V_Death_Nature")
  );
  ...
  const getdep = async () => {
    ...
    const response = await axiosJWT.get(
      `${process.env.REACT_APP_BACKEND_URL}/viewformwdep`,
      {
  ...
  const ViewFormWEdit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/ViewFormWEdit`, {
  ...
  const onESChange = (e) => {
    ...
    if (
      ...
      de.test(e.target.value.toUpperCase())
    ) {
            ...
            <form>
              ...
              <div className="upperForm1Content">
                <div className="row">
                  ...
                  <div className="col-lg-4 space">
                    ...
                    />
                    <br />
                    <span style={{ color: "red" }}>{ESMErr}</span>
                  </div>
                  ...
                  <div className="col-lg-4 space">
                    <input
                      ...
                      name="Death_Nature "
                      value={Death_Nature}
                      onChange={(e) => setDeath_Nature(e.target.value)}
                      required
                    />
                  </div>
                  ...
                  <div className="col-lg-4 space">
                    ...
                    />
                    <br />
                    <span style={{ color: "red" }}>{FFPErr}</span>
                  </div>
                </div>
              </div>
              ...
              >
                <button className=" btn">
                  <Link to="/ViewFormWa">Back</Link>{" "}
                </button>
                <button className="btn my-2 my-sm-0 " onClick={ViewFormWEdit}>
                  Update{" "}
                </button>
              </div>
            </form>
